...
pub fn selftest() {
    ...
    let nonce = [0x00, 0x00, 0x00, 0x09,
                 ...
                 0x00, 0x00, 0x00, 0x00];
    let counter = 1;
    let block = [0x10, 0xf1, 0xe7, 0xe4, 0xd1, 0x3b, 0x59, 0x15,
    ...
    selftest_noinline(&key, &nonce, counter, &block);
}

#[inline(never)]
#[cold]
fn selftest_noinline(key: &[u8], nonce: &[u8], counter: u32, expected: &[u8]) {
    let mut state = ChaCha20::with_counter(key, nonce, counter);
    assert_eq!(state.next().as_bytes(), expected);
}
